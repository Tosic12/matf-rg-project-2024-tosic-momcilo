...
void MainController::poll_events() {
    ...
    m_current_time += dt;
    if (!m_mined && !m_app && platform->key(engine::platform::KEY_M)
                                      .state() == engine::platform::Key::State::JustPressed) {
        m_mined = true;
        m_current_time = 0;
    } else if (m_mined && m_current_time >= m_mine_to_app) {
        std::default_random_engine engine;
        engine.seed(std::chrono::system_clock::now().time_since_epoch().count());
        std::uniform_real_distribution<float> dis_x(-0.3, -0.25), dis_y(-0.1, 0.1), dis_z(0.2, 0.3);
        m_mined_pos = glm::vec3{dis_x(engine), dis_y(engine), dis_z(engine)};
        m_mined = false;
        m_app = true;
        m_current_time = 0;
    } else if (m_app && m_current_time < m_app_to_steal) {
        glm::vec3 dir = m_spaceship_pos - m_mined_pos;

        if (length(dir) < 0.01f) {
            // the gem has been mined succsesfully
            m_app = false;
            m_mined_cnt += 1;
            spdlog::info("MINED %d", m_mined_cnt);
        }

        // The gem should be approaching slower when it is close, but up to a limit
        if (length(dir) < 0.2f) { dir = normalize(dir) * 0.2f; }
        m_mined_pos += dt * dir * m_mined_speed;
    } else if (m_app && m_current_time >= m_app_to_steal) { m_app = false; }
}
...
void MainController::draw_model(engine::resources::Shader *shader, const std::string &name, const std::string &texture_name, const std::string &path, const glm::vec3 &pos, const glm::vec3 &rot, const glm::vec3 &scale, int32_t texture_index) {
    ...
    shader->set_vec3("material.specular", glm::vec3{m_mat_specular_comp});
    shader->set_float("material.shininess", m_mat_shininess_comp);
    model->draw(shader);
}

void MainController::draw_mined_diamond(engine::resources::Shader *shader) { if (m_app) { draw_model(shader, "diamond", "green_gem", "", m_mined_pos, glm::vec3{0}, m_mined_scale, 3); } }
...
void MainController::draw_gui_settings() {
    ...
    graphics->begin_gui(); {
        ...
        ImGui::Begin("GUI panel");

        ImGui::Text("Mined %d", m_mined_cnt);
...
void MainController::update_camera() {
    ...
    if (platform->key(engine::platform::MOUSE_BUTTON_RIGHT)
                .state() == engine::platform::Key::State::Pressed) { camera->rotate_camera(mouse.dx, mouse.dy); }
...
void MainController::update_spaceship() {
    ...
    if (platform->key(engine::platform::KEY_LEFT)
                .state() == engine::platform::Key::State::Pressed) { delta_pos.x -= dt_space; }
    if (platform->key(engine::platform::KEY_RIGHT)
                .state() == engine::platform::Key::State::Pressed) { delta_pos.x += dt_space; }
    if (platform->key(engine::platform::KEY_Q)
                .state() == engine::platform::Key::State::Pressed) { delta_pos.y -= dt_space; }
    if (platform->key(engine::platform::KEY_E)
                .state() == engine::platform::Key::State::Pressed) { delta_pos.y += dt_space; }
    if (platform->key(engine::platform::KEY_UP)
                .state() == engine::platform::Key::State::Pressed) { delta_pos.z -= dt_space; }
    if (platform->key(engine::platform::KEY_DOWN)
                .state() == engine::platform::Key::State::Pressed) { delta_pos.z += dt_space; }
    if (glm::length(m_spaceship_pos + delta_pos) >= 0.245f) {
        // distance from the asteroid
        m_spaceship_pos += delta_pos;
    }
    m_fuelball_scale += m_fuel_sign * dt_space / 10000;
    if (m_fuelball_scale.x > m_fuelball_init_scale * 1.1f) { m_fuel_sign = -1; }
    if (m_fuelball_scale.x < 0.9f * m_fuelball_init_scale) { m_fuel_sign = 1; }
}
}
